...
// -------------------------------------------------------------------------
// هنبدأ بعرض أول 10 صور
let currentGalleryCount = 10;
// كل دوسة هتزود 6 صور (أو أي رقم تحبه)
const GALLERY_INCREMENT = 6;
// حط رقم كبير جداً هنا عشان نضمن إنه يغطي كل صورك المستقبلية
const MAX_POSSIBLE_IMAGES = 5000; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(document.getElementById('gallery-grid')) initGalleryPage(); // هنا التعديل المهم
...
// -------------------------------------------------------------------------
// بيانات الكورسات (نفس البيانات اللي ضفناها المرة اللي فاتت)
// -------------------------------------------------------------------------
const coursesData = [
    // ... (كل الكورسات الـ 45 اللي ضفناهم موجودين هنا، بس عشان المساحة مش هكررهم تاني، هم محفوظين في النسخة اللي فاتت، بس لازم يكونوا موجودين في الملف النهائي)
    // --- (افترض إن هنا قائمة الكورسات كاملة) ---
    // سأضع عينة فقط لعدم التكرار، لكن عليك وضع القائمة الكاملة هنا
    { 
        id: 246, 
...
function initGalleryPage() { 
    // لو أول مرة نفتح، بنبدا بـ 10 صور
    ...
    if(loadMoreBtn) {
        // فك الارتباط القديم (عشان التكرار) وإضافة الجديد
        ...
        newBtn.addEventListener('click', () => {
            ...
            // ارسم الجاليري من تاني بالعدد الجديد
            renderGallery();
        });
    }
}
...
function renderGallery() {
    ...
    // بنفضي الجريد القديم ونعيد رسمه بالكامل عشان الترتيب يظبط
    let html = '';
        ...
        <div class="glass-panel rounded-2xl overflow-hidden break-inside-avoid mb-6 group relative fade-in border-0 shadow-sm" id="img-container-${i}">
            <div class="relative cursor-pointer" onclick="openLightbox('images/${i}.jpg')">
                <img 
                    ...
                    onerror="this.parentElement.parentElement.style.display='none'; this.parentElement.parentElement.classList.add('hidden-img');"
                >
    ...
    // تحديث الأيقونات واللايكات
    lucide.createIcons();
    listenToLikes();

    // تحديث زرار عرض المزيد (نص)
